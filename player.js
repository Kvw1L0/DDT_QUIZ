...
// BLINDAJE 1: Manejo seguro de LocalStorage
let myName = "";
...
} catch(e) { console.warn("Estadísticas reseteadas por error de formato"); }
...
// AUTO-LOGIN SI YA ESTÁ REGISTRADO
if(myName) {
    avanzarAlLobby();
}
...
UI.joinBtn.addEventListener('click', () => {
    ...
    // BLINDAJE 2: Avanzar la UI de inmediato sin esperar promesas
    avanzarAlLobby();
    
    // BLINDAJE 3: Pedir permisos de sensor en segundo plano (iOS)
    if (typeof DeviceOrientationEvent !== 'undefined' && typeof DeviceOrientationEvent.requestPermission === 'function') {
        DeviceOrientationEvent.requestPermission().catch(() => { console.log("Permiso de giroscopio denegado o ignorado."); });
    }
});
...
function avanzarAlLobby() {
    ...
    // Estado inicial visual para dar tranquilidad al usuario
...
function start() {
    onValue(ref(db, 'game'), (snap) => {
        ...
        // Si la base de datos está vacía (Admin aún no inicia)
        if(!currentGameData) {
            resetUI();
        ...
        if(s === 'lobby') {
            ...
            if(isCorrect) {
                ...
                sfxWin.play().catch(()=>{}); // Catch por si el navegador bloquea autoplay
